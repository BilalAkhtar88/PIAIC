...
export async function displayWelcomeMenu() {
    ...
    */
    ...
    const { choice } = await inquirer.prompt({   //This is called destructuring
        type: "list",
        name: "choice",
...
export async function checkBalance(user: any) {
    console.log(chalk.yellow.bold(`\nYour balance: ${user.balance}\n`));
}

export async function depositMoney(user: any) {
...
export async function withdrawMoney(user: any) {
    const { amount } = await inquirer.prompt({
        type: "number",
        name: "amount",
